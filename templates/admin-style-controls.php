 ...
 *
 * Included by both templates/admin-style-settings.php (the global
 * defaults page) and templates/admin-style-overrides.php (the per-table
 * panel), and returns the closures they render with, so the two screens
 * are one implementation rather than two that drift.
 *
 * The caller supplies $kdna_context: 'global' for the settings page,
 * 'table' for the per-table panel. In table context every field gains an
 ...
 * per-section and whole-table reset actions.
 *
 * ── Addressing ────────────────────────────────────────────────────────
 *
 ...
 *
 * @var string $kdna_context 'global' or 'table'.
 *
 * @package KDNA_Tables
 */
...
$kdna_is_table = ( 'table' === $kdna_context );
 ...
 */
$kdna_render_field = static function ( array $definition, $key, $field_key, array $devices, array $icons, $nested = false ) use ( $kdna_render_leaf, $kdna_render_switcher, $kdna_is_table ) {
	$responsive = ! empty( $definition['responsive'] );
	$k          = esc_attr( $key );
	$f          = esc_attr( $field_key );
	?>
	<div class="kdna-style-field<?php echo $nested ? ' kdna-style-field--nested' : ''; ?>"
		<?php if ( $kdna_is_table ) : ?>:class="{ 'is-inherited': ! isOverridden( '<?php echo $k; ?>', '<?php echo $f; ?>' ) }"<?php endif; ?>>
		<div class="kdna-style-field__head">
			...
			<span class="kdna-style-field__tools">
				<?php if ( $responsive ) : ?>
					<span <?php if ( $kdna_is_table ) : ?>x-show="isOverridden( '<?php echo $k; ?>', '<?php echo $f; ?>' )"<?php endif; ?>>
						<?php $kdna_render_switcher( $key, $field_key, $devices, $icons ); ?>
					</span>
				<?php endif; ?>

				<?php if ( $kdna_is_table ) : ?>
					<?php
					/*
					 ...
					 * shows Revert, which clears the override and lets the
					 * global show through again.
					 */
					?>
					<button
						type="button"
						class="kdna-style-field__override"
						x-show="! isOverridden( '<?php echo $k; ?>', '<?php echo $f; ?>' )"
						@click="override( '<?php echo $k; ?>', '<?php echo $f; ?>' )"
						title="<?php esc_attr_e( 'Override the global default for this table', 'kdna-tables' ); ?>"
					><?php esc_html_e( 'Override', 'kdna-tables' ); ?></button>

					<button
						type="button"
						class="kdna-style-field__reset"
						x-show="isOverridden( '<?php echo $k; ?>', '<?php echo $f; ?>' )"
						@click="revert( '<?php echo $k; ?>', '<?php echo $f; ?>' )"
						title="<?php esc_attr_e( 'Drop this override and follow the global default again', 'kdna-tables' ); ?>"
					><?php esc_html_e( 'Revert to global', 'kdna-tables' ); ?></button>
				<?php else : ?>
					<button
						type="button"
						class="kdna-style-field__reset"
						x-show="hasValue( '<?php echo $k; ?>', '<?php echo $f; ?>' )"
						@click="resetControl( '<?php echo $k; ?>', '<?php echo $f; ?>' )"
		...
		<?php if ( $kdna_is_table ) : ?>
			...
			<p class="kdna-style-inherited" x-show="! isOverridden( '<?php echo $k; ?>', '<?php echo $f; ?>' )">
				<span class="kdna-style-inherited__label"><?php esc_html_e( 'Inheriting', 'kdna-tables' ); ?></span>
				<span class="kdna-style-inherited__value" x-text="inheritedLabel( '<?php echo $k; ?>', '<?php echo $f; ?>' )"></span>
			</p>
			<div x-show="isOverridden( '<?php echo $k; ?>', '<?php echo $f; ?>' )">
 ...
 *
 * In table context each section header gains a reset that clears every
 * override in it, so a user who has gone too far on one section can back
 * out of just that section rather than the whole table.
 */
$kdna_render_panel = static function ( array $sections, array $grouped, array $devices ) use ( $kdna_render_field, $kdna_device_icons, $kdna_is_table ) {
	?>
					...
					<div class="kdna-style-panel__header">
						...
						<?php if ( $kdna_is_table ) : ?>
							<button
								type="button"
								class="kdna-style-field__reset"
								x-show="sectionHasOverrides( '<?php echo esc_attr( $section_key ); ?>' )"
								@click="resetSection( '<?php echo esc_attr( $section_key ); ?>' )"
							><?php esc_html_e( 'Reset all in this section to inherit', 'kdna-tables' ); ?></button>
						<?php endif; ?>
					</div>
...
return array(
	'panel'       => $kdna_render_panel,
	'field'       => $kdna_render_field,
	'leaf'        => $kdna_render_leaf,
	'switcher'    => $kdna_render_switcher,
	'placeholder' => $kdna_placeholder,
